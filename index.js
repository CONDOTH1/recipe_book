...
var NoDataMessage = "Sorry no recipe avaliable";
...
var WelcomeBackMessage = "Welcome Back.";
  var noRecipeMessage = "You have not picked a recipe for today. Please choose one in the Reecipe Book App."
...
var newSessionHandlers = {
  'LaunchRequest': function () {
    ...
    ddb.getItem('Recipe_Book_Recipes', userId, null, {}, function(err, response, cap){
      var today = new Date();
...
var startSearchHandlers = Alexa.CreateStateHandler(states.SEARCHMODE, {
    ...
    'getSpecificIngredientIntent': function() {
      ...
      output = "You need " + this.attributes.ingredients.find(function(element){return element.toLowerCase().includes(specificIngredient.toLowerCase())}) + " for this recipe";
      if (output == undefined) {
    ...
    'startTimerIntent': function () {
      ...
      this.emit(':ask', timerStartMessage)
    },

    'timeLeftIntent': function () {
      var timerName = this.event.request.intent.slots.description.value;
      if (!this.attributes.timers[timerName]) {
        var output = "Sorry I couldn't find that timer."
      } else {
        var timeLeft = getTimeLeft(this.attributes.timers[timerName]);
        var output = makeTimeString(timeLeft) + timerName;
      }
      this.emit(':ask', output);
    },
...
function parseRecipe(recipeJSON){
  ...
  var mappedArray = details.map( function(element) { return element.Description } );
  var joinedRecipe = mappedArray.join();
  var splitRecipe = joinedRecipe.match(/[A-Z].+?[\.\ \, \n]+?(?=[A-Z][a-z]|$)/g);
  var mappedArrayWithSteps = splitRecipe.map(function(element, index) {return "Step" + " " + (index + 1) + ", " + element.trim()});
